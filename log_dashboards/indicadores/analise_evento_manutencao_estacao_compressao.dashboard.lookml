---
- dashboard: anlise_de_eventos_de_manuteno__estao_de_compresso
  title: ANÁLISE DE EVENTOS DE MANUTENÇÃO - ESTAÇÃO DE COMPRESSÃO
  layout: newspaper
  preferred_viewer: dashboards-next
  description: ''
  preferred_slug: Z6UvfpWkbaTI8hk5fRvght
  elements:
  - title: Total de Eventos no Mês
    ...
    listen:
      ...
      Estação de Compressão: instalacoes_abrv.concat_name
  ...
  - title: Comparativo Mensal vs Anual de Eventos
    ...
    type: single_value
    fields: [contagem_manutencao.ano, contagem_manutencao.instalacao, contagem_manutencao.estacao_compressao,
      contagem_manutencao.sum_estacao_compressao_mes]
    sorts: [contagem_manutencao.instalacao desc]
    limit: 500
    column_limit: 50
    dynamic_fields:
    - category: table_calculation
      expression: if(${contagem_manutencao.estacao_compressao}!= 0,${contagem_manutencao.sum_estacao_compressao_mes}
        / ${contagem_manutencao.estacao_compressao}, 0)
      label: razao
      value_format:
      ...
      _type_hint: number
    - category: table_calculation
      expression: "if(sum(${razao}) = count(${contagem_manutencao.sum_estacao_compressao_mes})\
        \ * 1, \n  1.0,\n  sum(${contagem_manutencao.sum_estacao_compressao_mes})/\
        \ sum(${contagem_manutencao.estacao_compressao}))"
      label: razao_final
      value_format:
      value_format_name: percent_2
      _kind_hint: measure
    ...
    hidden_fields: [contagem_manutencao.ano, contagem_manutencao.instalacao, razao,
      contagem_manutencao.sum_estacao_compressao_mes, contagem_manutencao.estacao_compressao]
    listen:
  ...
  - name: Mês
    ...
    model: cortex_sap_operational
    explore: analise_eventos
    listens_to_filters: [Ano, Estação de Compressão, Tipo Instalação]
  ...
  - name: Ano
    ...
    model: cortex_sap_operational
    explore: analise_eventos
    listens_to_filters: [Estação de Compressão, Tipo Instalação]
    field: nota_manutencao_zspmlistnote.ano_nota
  - name: Estação de Compressão
    title: Estação de Compressão
    type: field_filter
    default_value: ''
    allow_multiple_values: true
    required: false
